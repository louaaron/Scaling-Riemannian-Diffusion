...
from riemdiffpp import HyperSphere, get_sm_loss
...
def run_train(cfg, work_dir, wandb_name):
    ...
    train_set = dataset.get_data(cfg.root, cfg.data, "train", dim=cfg.size)
    val_set = dataset.get_data(cfg.root, cfg.data, "valid", dim=cfg.size)
